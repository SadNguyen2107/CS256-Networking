...
//* VALIDATE StudentName_ID 
bool validateStudentName_ID(std::string studentName_ID)
{
    return std::regex_match(studentName_ID, std::regex("([A-Za-z_]+) - (\\d{8})"));
}

//* VALIDATE HUST-ID
bool validateHustID(std::string hust_ID)
{
    return std::regex_match(hust_ID, std::regex("\\d{8}"));
}
